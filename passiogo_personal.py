...
class TransportationSystem:
    ...
    def checkTypes(self):
        ...
        assert (
            type(self.id) == int
        ), f"'id' parameter must be an int not {type(self.id)}"

        # name : str or None
        assert (
            type(self.name) == str or self.name is None
        ), f"'name' parameter must be a str not {type(self.name)}"

        # username : str or None
        assert (
            type(self.username) == str or self.username is None
        ), f"'username' parameter must be a str not {type(self.username)}"

        # goAgencyName : str or None
        assert (
            type(self.goAgencyName) == str or self.goAgencyName is None
        ), f"'goAgencyName' parameter must be a str not {type(self.goAgencyName)}"

        # email : str or None
        assert (
            type(self.email) == str or self.email is None
        ), f"'email' parameter must be a str not {type(self.email)}"

        # goTestMode : bool or None
        assert (
            type(self.goTestMode) == bool or self.goTestMode is None
        ), f"'goTestMode' parameter must be a bool not {type(self.goTestMode)}"

        # name2 : bool or None
        assert (
            type(self.name2) == bool or self.name2 is None
        ), f"'name2' parameter must be a bool not {type(self.name2)}"

        # homepage : str or None
        assert (
            type(self.homepage) == str or self.homepage is None
        ), f"'homepage' parameter must be a str not {type(self.homepage)}"

        # logo : bool or None
        assert (
            type(self.logo) == bool or self.logo is None
        ), f"'logo' parameter must be a bool not {type(self.logo)}"

        # goRoutePlannerEnabled : bool or None
        assert (
            type(self.goRoutePlannerEnabled) == bool
            or self.goRoutePlannerEnabled is None
        ), f"'goRoutePlannerEnabled' parameter must be a bool not {type(self.goRoutePlannerEnabled)}"

        # goColor : str or None
        assert (
            type(self.goColor) == str or self.goColor is None
        ), f"'goColor' parameter must be a str not {type(self.goColor)}"

        # goSupportEmail : str or None
        assert (
            type(self.goSupportEmail) == str or self.goSupportEmail is None
        ), f"'goSupportEmail' parameter must be a str not {type(self.goSupportEmail)}"

        # goSharedCode : int or None
        assert (
            type(self.goSharedCode) == int or self.goSharedCode is None
        ), f"'goSharedCode' parameter must be a int not {type(self.goSharedCode)}"

        # goAuthenticationType : bool or None
        assert (
            type(self.goAuthenticationType) == bool or self.goAuthenticationType is None
        ), f"'goAuthenticationType' parameter must be a bool not {type(self.goAuthenticationType)}"

    def getRoutes(self, appVersion=1, amount=1) -> list["Route"]:
        """
    ...
    def getStops(self, appVersion=2, sA=1, raw=False) -> list["Stop"]:
        """
        ...
        for id, stop in stops["stops"].items():
            ...
            for routeId in routesAndStops.keys():
                if stop["id"] not in routesAndStops[routeId]:
                    continue
                routesAndPositions[routeId] = [
                    i for i, x in enumerate(routesAndStops[routeId]) if x == stop["id"]
                ]
    ...
    def getSystemAlerts(
        self, appVersion=1, amount=1, routesAmount=0
    ) -> list["SystemAlert"]:
        """
    ...
    def getVehicles(self, appVersion=2) -> list["Vehicle"]:
        """
...
def printAllSystemsMd(includeHtmlBreaks=True):
    systems = getSystems()
...
class Route:
    ...
    def getStops(self):
        ...
        for stop in allStops:
            if (
                self.myid in list(stop.routesAndPositions.keys())
                or self.id in list(stop.routesAndPositions.keys())
                or self.groupId in list(stop.routesAndPositions.keys())
            ):
